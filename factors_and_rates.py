...
tel_gap = 25#15 #distance from first paddle to last paddle
tel_eff = 1.00
muon_flux = 1/(60*100)
...
x0 = np.random.uniform(-start_side_x/2.0, start_side_x/2.0, n)
y0 = np.random.uniform(-start_side_y/2.0, start_side_y/2.0, n)
...
tel = (np.abs(xtel) < end_side_x/2.0) & (np.abs(ytel) < end_side_y/2.0) \
...
xtel = x0 + tel_gap * tan_theta * cos_phi
ytel = y0 + tel_gap * tan_theta * sin_phi
p0 = (np.abs(x0) < start_side_x/2.0) & (np.abs(x0) < start_side_y/2.0) \
        & (np.random.rand(n) < tel_eff)
ptel = (np.abs(xtel) < end_side_x/2.0) & (np.abs(ytel) < end_side_y/2.0) \
        & (np.random.rand(n) < tel_eff)
...
print("Telescope rate: %.3E Hz" % (tel_eff * g * muon_flux * start_side_x * start_side_y))
